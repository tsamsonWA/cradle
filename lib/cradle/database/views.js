...
Database.prototype.csearch = function (path, options, callback) {
    if (typeof options === 'function') {
        callback = options;
        options = {};
    }
    ...
    path = path.split('/');
    path = ['_design', path[0], '_search', path[1]].map(querystring.escape).join('/');
    // console.log("--cradle---");
    // console.log(path);
    // console.log(options);
    // console.log("--/cradle---");
...
function parseOptions(options) {
    if (options && typeof options === 'object') {
        ['key', 'startkey', 'endkey', 'sort'].forEach(function (k) {
            if (k in options) { options[k] = JSON.stringify(options[k]) }
        });
    }
